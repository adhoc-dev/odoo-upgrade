...
def migrate(cr, version):
    ...
    active le va a tocar el dato a cada cliente que actualice. Sacarlo de aca antes de ese
    momento.

    El crawler de Odoo (`base/tests/test_mock_crawl`) abre cada menu con un usuario demo antes
    y despues del upgrade y falla si alguno dejo de andar. En la demo las 10 BOMs no tienen
    compania y sus operaciones apuntan a workcenters de Muebleria US. Como
    ...
    _logger.info("Set the company of %s company-less bom(s) from their workcenters", cr.rowcount)
