...
The first script to use ``add_customer_note``, and the shape to copy from.

Upgrade line 2021 computes these over the already migrated database, through odooly; here
the upgrade process itself computes them and leaves them for the note.
...
The keys are the variable names the message renders, so they stay in Spanish. With no
slug they are the whole link between the two sides, so renaming one here silently empties
what it fills in the message.
"""
...
def _invoiced_not_received(cr, env):
    ...
    # prepaid_expense is declared by purchase_accountant, not by purchase: gating on the
    # latter lets the search through on a base without account_accountant, and it raises.
    if not util.module_installed(cr, "purchase_accountant"):
        return []
    return [
        {
...
def _invoiced_not_delivered(cr, env):
    ...
    # deferred_revenue is declared by sale_account_accountant, not by sale.
    if not util.module_installed(cr, "sale_account_accountant"):
        return []
    return [
        {
            "orden": line.order_id.name,
