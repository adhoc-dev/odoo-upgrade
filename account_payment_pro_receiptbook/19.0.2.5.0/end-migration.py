...
Va como ``end`` y no como ``post``: el ``post`` corre dentro del loop de carga
por módulo, antes de que ``base/19.0.0.0/end-migration.py`` (que corre recién
en la STEP 3.5, después de todos los módulos) arme el árbol de compañías
(``parent_id``). Sin árbol armado, ``_resolve_branch_prefix_collisions()`` no
encuentra colisiones y no hace nada.
"""
