...
def patched_check_external_dependencies(cls, module_name, newstate='to install'):
    pass
...
def add_module_dependencies(cr, module_list):
    ...
    """
    _logger.info(
        'Running UPGRADE add_module_dependencies for module_list %s',
        module_list)
    ...
    while dependencies:
        ...
        _logger.info('Adding dependencies %s', module_list)
...
def load_marked_modules(cr, graph, states, force, progressdict, report,
    ...
    _logger.info('Running UPGRADE load_marked_modules')
